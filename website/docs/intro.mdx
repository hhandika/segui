...
Learrn more about using SEGUL API in Python [here](/docs/api-usage/python/intro).
:::
...
We provide quick start [below](#quick-start-%EF%B8%8F) to help you get started with SEGUL. The quick start assumes familiarity with phylogenomic data and some experience using related tools. Any users interested in installing SEGUL should be benefitted from our [installation guidelines](/docs/installation/overview). If you are using GUI app, we recommend checking the [GUI general usage](/docs/gui-usage/general) section. For using the CLI application, we recommend starting with the [CLI Overview](/docs/cli-usage/overview) section. These sections provide an overview of the app, input and output formats, datatype, and other general information about the app. If you have any questions, issues, and feedback about the app and its documentation, please refer to the [support](/docs/support) section.
...
If your answer is yes to any of the following questions, you should use the CLI version of the app. Otherwise, either GUI or CLI will serve your need. You can also use both versions. They each operates in their own contained environment.
...
```python
# <Your code using similar code structure as above...>

input_dir = 'tests/data'
# Or a list of files
input_files = ['tests/data/alignment1.nex', 'tests/data/alignment2.nex']
# <An exract object with its input paramaters>
# Then, extract the sequences using regular expression method
object_name.extract_regex("(?i)^(abce)")
```
