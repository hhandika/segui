...
Learn more about the differences between the CLI and GUI versions [here](./cli_gui.md).
...
For GUI apps, we recommend downloading from your operating system's [official store](/docs/installation/install_gui). The fastest installation route for CLI is using the pre-compiled binaries. If you are familiar with installing single executable CLI apps, download the latest release using the links below. The [installation guide](/docs/installation/install_binary) provides more detailed instructions.
...
:::warning
The commands below expect SEGUL version 0.19.0+. If you are using a lower version, the subcommand becomes the command, and the dir input option requires a specific input format.
