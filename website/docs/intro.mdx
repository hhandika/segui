...
:::info
SEGUL CLI is now available in [Bioconda](https://anaconda.org/bioconda/segul). Available to install for Linux and MacOS users using [ARM](https://en.wikipedia.org/wiki/ARM_architecture_family) or [x86_64](https://en.wikipedia.org/wiki/X86-64) CPUs.
...
Learn more about Bioconda installation [here](/docs/api-usage/python/intro).
:::
...
[Below](#quick-start-%EF%B8%8F), we provide a quick start to help you start with SEGUL. The quick start assumes familiarity with phylogenomic data and some experience using related tools. Detailed instructions on installing SEGUL are available in the [installation guidelines](/docs/installation/overview). We recommend checking the [GUI general usage](/docs/gui-usage/general) section if you use the GUI app. To use the CLI application, we recommend starting with the [CLI Overview](/docs/cli-usage/overview) section. These sections provide an overview of the app, input and output formats, datatype, and other general information about the app. Please refer to the [support](/docs/support) section for questions, issues, or feedback about the app and its documentation. We also welcome collaboration and contribution.

:::tip
For mobile reading, you can find a dropdown menu at the top of each page to navigate to different documentation sections. For larger screen reading, the sidebar on the right side of the page provides the same functionality.
:::
...
For general users, the choice will be between the command-line (CLI) and graphical interface (GUI) applications. If you answer yes to any of the following questions, you should use the CLI version of the app. Otherwise, either GUI or CLI will serve your needs. You can also install both versions and use them interchangeably. They operate in their environment and do not interfere with the other.
...
For developers, SEGUL is available as a Rust crate, which you can easily integrate into Rust code. The Python library is available as a Python binding to the SEGUL API called PySEGUL. It is user-friendly and does not require Rust knowledge to use it. The Rust crate also works with R. However, using it in R requires some Rust knowledge.
...
For GUI apps, we recommend downloading from the [official store](/docs/installation/install_gui) of your operating system. The fastest installation route for CLI is using the pre-compiled binaries. If you are familiar with installing single executable CLI apps, download the latest release using the links below. The [installation guide](/docs/installation/install_binary) provides more detailed instructions.
...
4. You can add the input files by clicking the "Add file" button. On desktop platforms, you can also input a directory by clicking the "Add directory" button. The app will look for files matching the directory.
